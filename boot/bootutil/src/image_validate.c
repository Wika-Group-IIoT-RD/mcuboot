 ...
 */
     ...
     defined(MCUBOOT_SIGN_EC384) + defined(MCUBOOT_SIGN_ED25519)) > 1
...
fih_ret bootutil_img_validate(struct boot_loader_state *state,
    ...
    defined(MCUBOOT_HW_ROLLBACK_PROT) || defined(MCUBOOT_UUID_VID) ||          \
    defined(MCUBOOT_UUID_CID)
  ...
  img_sz = it.tlv_end;
#endif
  BOOT_LOG_DBG("bootutil_img_validate: TLV off %u, end %u", it.tlv_off,
               it.tlv_end);
  ...
  while (true) {
    ...
    case EXPECTED_SIG_TLV: {
      ...
      FIH_CALL(bootutil_verify_sig, valid_signature, hash, sizeof(hash), buf,
               len, key_id);
#else
       ...
       */
      FIH_CALL(bootutil_verify_img, valid_signature,
               (void *)(base + flash_area_get_off(fap)),
    ...
    case IMAGE_TLV_SEC_CNT: {
      ...
      if (FIH_NOT_EQ(fih_rc, FIH_SUCCESS)) {
        BOOT_LOG_ERR(
            "Image security counter value %u lower than monotonic value %u",
            img_security_cnt, (uint32_t)fih_int_decode(security_cnt));
        FIH_SET(fih_rc, FIH_FAILURE);
        goto out;
      }
      ...
      if (img_security_cnt > (uint32_t)fih_int_decode(security_cnt)) {
        FIH_CALL(boot_nv_security_counter_is_update_possible, fih_rc,
                 image_index, img_security_cnt);
        if (FIH_NOT_EQ(fih_rc, FIH_SUCCESS)) {
          FIH_SET(fih_rc, FIH_FAILURE);
          BOOT_LOG_ERR(
              "Security counter update is not possible, possibly the maximum "
